...
def run(category_words, images, background, click_sound, limb_sound):
    ...
    # I start the game theme when the round begins
    ...
    hangman_status = 0
    word = random.choice(category_words)  # I pick a word from the selected category
    guessed = []
    bloody_red = (139, 0, 0)
    bone_white = (245, 245, 220)

    # I prepare the letter buttons
    radius = 20
    gap = 15
    letters = []
    ...
    def draw():
        # I draw the background and current skeleton stage
        ...
        # I draw the word with guessed letters
    ...
    def end_screen(message):
        # I show the end screen with options
        while True:
    ...
    def handle_loss():
        limb_sound.stop()
        pygame.mixer.music.stop()
        # Show final skeleton image first
        draw()
        pygame.display.update()
    ...
    # I run the main loop for a round
    while running:
        clock.tick(FPS)
        draw()
